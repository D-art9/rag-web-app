...
const LandingPage: React.FC<LandingPageProps> = ({ onUrlSubmit, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUploadResult({ url, id: response.id });
      setIsUploadFinished(true); // Triggers LoadingScreen to go to 100%
    } catch (err) {
      const errorObj: any = err;
      setError(errorObj.response?.data?.message || 'Failed to analyze video. Please check the URL and try again.');
      setIsProcessing(false); // Hide loading screen immediately on error
    }
  };

  const handleLoadingComplete = () => {
    if (uploadResult) {
      onUrlSubmit(uploadResult.url, uploadResult.id);
    }
  };
